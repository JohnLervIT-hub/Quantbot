...
const STRATEGY_SESSION_MATRIX = {
  TOKYO:  { best: "Mean Revert",  second: "Range Scalp",  avoid: ["Trend Follow", "Breakout"] },
  SYDNEY: { best: "Range Scalp",  second: "Mean Revert",  avoid: ["Trend Follow", "Breakout"] },
  LONDON: { best: "Trend Follow", second: "Breakout",     avoid: ["Range Scalp"] },
  PRIME:  { best: "Trend Follow", second: "Breakout",     avoid: [] },
  NY:     { best: "Momentum",     second: "Trend Follow", avoid: ["Range Scalp", "Mean Revert"] },
  AVOID:  { best: "Mean Revert",  second: "Range Scalp",  avoid: ["Trend Follow", "Breakout", "Momentum"] },
};
...
function useStrategyIntelligence({ strategy, closedTrades, openTrades, balance, globalRegime }) {
  ...
  const compute = useCallback(() => {
    ...
    if (heat >= 5) {
      ...
      recommended  = matrix.best;
      reason       = `${sess} session — ${matrix.best} performs best in this window`;
      notifType    = "session";
      ...
      if (trades.length >= 10) {
        ...
        if (curPerf && curPerf.expectancy < 0 && curPerf.tradeCount >= 5 && recommended === cur) {
          recommended = matrix.second || matrix.best;
