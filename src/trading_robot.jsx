...
function SwingConsensusPanel({ pair, sig, session, xavierIntel, freshNews, livePrices, onExecute, onCancel }) {
  ...
  const confirmers     = consensus?.models?.filter(m => m.verdict === "CONFIRM").map(m => m.name).join(" + ") ?? "";
  const dissenters     = consensus?.models?.filter(m => m.verdict === "REJECT").map(m => m.name).join(" + ") ?? "";
  const MODEL_PLACEHOLDERS = ["WARREN", "GEORGE", "JAMES", "RAY"];
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: accent, letterSpacing: "0.04em" }}>
          {loading
            ...
            : executeAllowed
              ? `⚔️ Kill Shot cleared — ${confirmers} confirmed`
              : `⚔️ BLOCKED — ${dissenters ? dissenters + " rejected" : "insufficient votes"}`}
        </div>
        {!loading && consensus && (
  ...
  return (
    <div style={{ margin: isMobile ? "0 0 16px" : "0 16px 16px", background: "#161b22", border: "1px solid #F9731640", borderRadius: 12, overflow: "hidden" }}>
      ...
      {activePairs.map(pair => {
        ...
        const csConfirmers  = cs?.data?.models?.filter(m => m.verdict === "CONFIRM").map(m => m.name).join(" + ") ?? "";
        const csDissenters  = cs?.data?.models?.filter(m => m.verdict === "REJECT").map(m => m.name).join(" + ") ?? "";
              ...
              {cs?.loading ? (
                ...
                <>
                  ...
                  <div style={{ borderTop: "1px solid #21262d", marginTop: 7, paddingTop: 6, fontSize: 10, fontWeight: 700, color: resultColor, letterSpacing: "0.03em" }}>
                    {executeAllowed
                      ? `Result: ${confirms}/4 — EXECUTING (${csConfirmers} confirmed${csDissenters ? " · " + csDissenters + " dissented" : ""})`
                      : `Result: BLOCKED — ${csDissenters ? csDissenters + " rejected" : "insufficient votes"}`}
                  </div>
                </>
              ) : !cs ? (
...
const MOBILE_TABS = [
  ...
  { key: "diagnostics", label: "Diag",  Icon: ({ color }) => <svg viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="20" height="20"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M4.22 4.22l2.12 2.12M17.66 17.66l2.12 2.12M2 12h3M19 12h3M4.22 19.78l2.12-2.12M17.66 6.34l2.12-2.12"/></svg> },
...
export default function TradingRobot() {
  ...
  const tabs = [["markets", "Markets"], ["news", "News"], ["ai", "Ask Xavier"], ["knowledge", "Knowledge"], ["risk", "Risk"], ["coach", "Coach"], ["history", "History"], ["analytics", "Analytics"], ["diagnostics", "Diagnostics"], ["schedule", "Schedule"], ["backtest", "Backtest"]];
